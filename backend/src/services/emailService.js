...
/**
 * Gera o template HTML do e-mail de verificação de conta (Signup OTP)
 * @param {string} userName - Nome do usuário
 * @param {string} code - Código OTP de 4 dígitos
 * @returns {string} HTML formatado
 */
const buildVerificationEmailHTML = (userName, code) => {
  const digits = code.split('');
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Verificação de Conta — EduTrack AI</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f9fafb; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  <!-- Hack de Pré-Visualização (Anti-Preview Snippet) -->
  <div style="display: none; max-height: 0px; max-width: 0px; opacity: 0; overflow: hidden; font-size: 1px;">
    Bem-vindo ao EduTrack AI! Confirme seu e-mail para ativar sua conta.
  </div>
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #f9fafb; padding: 40px 0;">
    <tr>
      <td align="center">
        <table role="presentation" width="480" cellspacing="0" cellpadding="0" style="background-color: #ffffff; border-radius: 16px; box-shadow: 0 4px 24px rgba(0,0,0,0.06); overflow: hidden;">
          ...
          <tr>
            <td style="background-color: #ffffff; padding: 32px 40px 16px; text-align: center;">
              <img src="https://i.imgur.com/f3E22Wx.png" alt="EduTrack AI Logo" style="max-width: 160px; height: auto; display: block; margin: 0 auto; padding-top: 12px;" />
              <h2 style="color: #0f172a; font-size: 18px; font-weight: 700; margin: 16px 0 0 0; text-transform: uppercase; letter-spacing: 0.5px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">Verificação de Conta</h2>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 16px 40px 24px;">
              <p style="color: #1f2937; font-size: 16px; margin: 0 0 12px 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">Bem-vindo(a), <strong>${userName || 'Estudante'}</strong>!</p>
              <p style="color: #4b5563; font-size: 14px; line-height: 1.6; margin: 0 0 24px 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                Sua conta foi criada com sucesso! Para ativá-la, insira o código de verificação abaixo na plataforma:
              </p>
...
/**
 * Envia o e-mail de verificação de conta com o código OTP
 * @param {string} toEmail - E-mail do destinatário
 * @param {string} userName - Nome do usuário para personalização
 * @param {string} code - Código OTP de 4 dígitos
 */
const sendVerificationCodeEmail = async (toEmail, userName, code) => {
  const transporter = createTransporter();
  const fromAddress = process.env.SMTP_FROM || 'noreply@edutrack.app';
  ...
  const mailOptions = {
    from: `"EduTrack AI" <${fromAddress}>`,
    to: toEmail,
    subject: 'EduTrack AI — Código de Verificação de Conta',
  ...
  if (!transporter) {
    console.log('\n╔══════════════════════════════════════════════════════════╗');
    console.log('║  📧 FALLBACK DE E-MAIL — VERIFICAÇÃO DE CONTA          ║');
    console.log('╠══════════════════════════════════════════════════════════╣');
    console.log(`║  Para: ${toEmail}`);
    console.log(`║  Usuário: ${userName || 'N/A'}`);
    console.log(`║  Código OTP: ${code}`);
  ...
  try {
    const info = await transporter.sendMail(mailOptions);
    console.log(`[EmailService] ✅ E-mail de verificação enviado para ${toEmail} — MessageId: ${info.messageId}`);
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error(`[EmailService] ❌ Falha ao enviar e-mail de verificação para ${toEmail}:`, error.message);
    console.log(`[EmailService] 🔄 Fallback ativado — Código OTP para ${toEmail}: ${code}`);
    return { success: true, fallback: true, error: error.message };
  }
};

module.exports = { sendResetCodeEmail, sendVerificationCodeEmail };
